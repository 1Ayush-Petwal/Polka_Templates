...
describe("MyToken", function () {
  ...
  beforeEach(async function () {
    ...
    if (signers.length === 0) {
      throw new Error(
        [
          "No signers available for this network.",
          "If you're running with --network polkadotTestnet, set TESTNET_PRIVATE_KEY (e.g. `npx hardhat vars set TESTNET_PRIVATE_KEY`).",
          "Otherwise, run tests on the default Hardhat network (omit --network).",
        ].join(" ")
      );
    }

    // Owner comes from the configured private key / network accounts
    owner = signers[0];
